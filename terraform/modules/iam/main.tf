# IAM Module for Medical AI Search Platform
# EKS cluster and node group roles with security policies
...
# Application Service Account Roles (for IRSA)
resource "aws_iam_role" "app_s3_access" {
  name = "${var.project_name}-${var.environment}-app-s3-access-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${var.oidc_provider_url}:sub" = "system:serviceaccount:default:medical-ai-app"
            "${var.oidc_provider_url}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
resource "aws_iam_policy" "app_s3_access" {
  name        = "${var.project_name}-${var.environment}-app-s3-access-policy"
